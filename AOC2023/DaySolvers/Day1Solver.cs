...
public class Day1Solver : IDaySolver
{
	...
	public long SolvePart1(string input)
	{
		...
		var calibrations = inputLines.Select(GetCalibration1).ToList();
		var result = calibrations.Sum();
		return result;
	}
	public long SolvePart2(string input)
	{
		var inputLines = input.Split(Environment.NewLine, StringSplitOptions.RemoveEmptyEntries);
		var calibrations = inputLines.Select(GetCalibration2).ToList();
		var result = calibrations.Sum();
		return result;
	}
	...
	private static long GetCalibration1(string inputline, int arg2)
	{
		...
		var lastNumber = GetNumber(last);

		var varcalibration = firstNumber * 10 + lastNumber;
