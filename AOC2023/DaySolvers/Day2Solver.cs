...
namespace AOC2023.DaySolvers
{
	public class Day2Solver : IDaySolver
	{
		...
		public long SolvePart1(string input)
		{
			...
			var results = games.Where(GameIsPossible).Sum(game => game.Key);
			return results;
		}
		...
		{
			var games = GetGames(input);
			var maxCubes = games.Select(GetMaxCubesPerGame).ToArray();
			var result = maxCubes
				.Select(maxCubes => maxCubes.Item1 * maxCubes.Item2 * maxCubes.Item3)
				.Sum();
			return result;
		}

		private enum CubeCollor
		{
			red,
			green,
			blue
		...
		private readonly Dictionary<CubeCollor, long> MaxCubes = new()
		{
			{ CubeCollor.red, 12 },
			{ CubeCollor.green, 13 },
			{ CubeCollor.blue, 14 }
		...
		private bool GameIsPossible(KeyValuePair<int, Dictionary<CubeCollor, int>[]> pair)
		{
			var rounds = pair.Value;
			var allRoundsArePossible = rounds.All(RoundIsPossible);
			return allRoundsArePossible;
		}

		private bool RoundIsPossible(Dictionary<CubeCollor, int> dictionary)
		{
			var allCubesArePossible = dictionary.All(CubeIsPossible);
			return allCubesArePossible;
		}

		private bool CubeIsPossible(KeyValuePair<CubeCollor, int> pair)
		{
			var isPossible = pair.Value <= MaxCubes[pair.Key];
			return isPossible;
		}

		private static Dictionary<int, Dictionary<CubeCollor, int>[]> GetGames(string input)
		{
			var lines = input.Split(Environment.NewLine, StringSplitOptions.RemoveEmptyEntries);
			var games = lines.ToDictionary(GetGameNumber, GetGame);
			return games;
		}
		...
		private static int GetGameNumber(string line)
		{
			...
			var gameNumber = int.Parse(game.Split(" ")[1].TrimEnd(':'));
			return gameNumber;
		}

		private static Dictionary<CubeCollor, int>[] GetGame(string line)
		{
			var gameAndData = line.Split(": ");
			var data = gameAndData[1];
			var rounds = GetRounds(data);
			return rounds;
		}

		private static Dictionary<CubeCollor, int>[] GetRounds(string data)
		{
			var roundsData = data.Split("; ");
			var rounds = roundsData.Select(GetRound).ToArray();
			return rounds;
		}

		private static Dictionary<CubeCollor, int> GetRound(string roundData)
		{
			var cubesData = roundData.Split(", ");
			var cubes = cubesData.ToDictionary(
				data => Enum.Parse<CubeCollor>(data.Split(" ")[1]),
				data => int.Parse(data.Split(" ")[0]));
			return cubes;
		}

		private (long, long, long) GetMaxCubesPerGame(KeyValuePair<int, Dictionary<CubeCollor, int>[]> pair, int arg2)
		{
			var rounds = pair.Value;
			var maxRedCubes = rounds
				.Where(round => round.ContainsKey(CubeCollor.red))
				.Max(round => round[CubeCollor.red]);
			var maxGreenCubes = rounds
				.Where(round => round.ContainsKey(CubeCollor.green))
				.Max(round => round[CubeCollor.green]);
			var maxBlueCubes = rounds
				.Where(round => round.ContainsKey(CubeCollor.blue))
				.Max(round => round[CubeCollor.blue]);
			var maxCubes = (maxRedCubes, maxGreenCubes, maxBlueCubes);
			return maxCubes;
		}
